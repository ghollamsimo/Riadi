...
const StepThree = ({ handlePrev, handleNext, loader, repas, handleChange , handleRepas}) => {
                            ...
                            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                {service?.datalist?.map(item => (
                                    <div key={item.id} className="flex text-sm sm:text-base ">
                                        <input id={item.id} name="service_id[]" onChange={handleCheck} type="checkbox" multiple value={item.id} className="focus:ring-action-primary h-6 w-6 text-primary-500 border-primary rounded border-neutral-500 bg-[#6366F1] dark:bg-[#6366F1]  dark:checked:bg-primary-500 focus:ring-primary-500" />
                        ...
                        <div>
                            ...
                            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                {repaData?.datalist?.map(item => (
                                    <div key={item.id} className="flex text-sm sm:text-base ">
                                        <input id={item.id} name="repa_id[]" multiple onChange={handleCheckRepas} type="checkbox" value={item.id} className="focus:ring-action-primary h-6 w-6 text-primary-500 border-primary rounded border-neutral-500 bg-[#6366F1] dark:bg-[#6366F1]  dark:checked:bg-primary-500 focus:ring-primary-500" />
                                        <label htmlFor={item.name} className="ml-3.5 flex flex-col flex-1 justify-center">
                                            <span className="text-neutral-900 dark:text-neutral-100">{item.name}</span>
                                        </label>
                                    </div>
                                ))}
